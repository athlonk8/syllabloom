...
B 站播放器提供“登录 B 站”“刷新播放器”“在 B 站高清播放”和页面内全屏按钮。“登录 B 站”会在 B 站自己的页面中完成，Syllabloom 不会读取、保存或代理密码和 Cookie。登录后可刷新嵌入播放器并选择 B 站允许的清晰度；如果浏览器阻止第三方 Cookie 或该视频的清晰度受账号/版权限制，可使用“在 B 站高清播放”作为同账号的一方页面回退。
