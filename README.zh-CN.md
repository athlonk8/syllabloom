...
新版图文说明：[功能图文说明](docs/FEATURE_TOUR.zh-CN.md)。其中包含 B 站扫码登录与清晰度切换的流程。
...
- 添加课程默认支持粘贴 B 站链接：应用解析 BV 号并读取公开分 P 信息，多 P 视频自动拆分为多个讲次；播放由内置 HTML5 播放器完成，支持扫码登录与清晰度切换。此类来源会明确标为学习者选择的第三方内容，不会被表述为课程官方材料。
...
- 不绕过 B 站的任何访问限制。B 站播放使用学习者本人账号的访问权限：登录通过 B 站官方扫码接口完成，Cookie 仅保存在本地数据库；媒体数据只在本机内存中转发，不下载、不保存副本，也不解锁账号权限之外的内容。
- 不会把 AI 编造的内容标为官方作业。
- 不会静默上传你的笔记、答案或课程资料。
...
4. 导入公开课程：默认粘贴 B 站视频链接（多 P 自动拆分），也可使用 YouTube Data API 导入或 Stanford 课程网址；已有讲次仍可展开「使用 B 站来源」更换为 B 站视频。
...
B 站播放由 Syllabloom 自带的 HTML5 播放器完成，不再依赖 B 站外链 iframe（外链 iframe 受浏览器第三方 Cookie 分区限制，登录后也无法获得更高清晰度）。点击「扫码登录」后，页面内会显示 B 站官方二维码，用手机 B 站客户端扫码即可完成登录；登录凭据（Cookie）只写入本机 SQLite 数据库，不会展示、上传或代理给任何第三方，设置接口也只返回「已配置」状态。清晰度菜单列出 B 站为你的账号实际返回的档位，切换时保持播放进度；未登录时通常只有 360P/480P，登录后可到你的账号有权观看的最高档位（大会员档位需要相应会员权限）。观看进度按不重复区间计入课程完成度，与 YouTube 讲次一致。
