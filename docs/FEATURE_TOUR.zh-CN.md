...
此截图在本地模拟了登录完成后的回跳路径，并未使用任何 B 站账号。实际可选清晰度仍由 B 站、视频和账号决定。Syllabloom 刻意不再提供跳转到 B 站站外播放的按钮；iframe 不包含弹窗或外层导航权限，因此不能把外层学习页面带离课程。若 B 站播放器本身针对嵌入播放显示“进入哔哩哔哩，观看更高清”的提示，该提示仍会显示，但不能带走外层课程页面。
