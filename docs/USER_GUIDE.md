...
The embedded Bilibili player requests Bilibili's high-quality mode, but its external player decides which streams it will actually serve. Use **Sign in and return to player** to open Bilibili's own first-party login page in the current tab. Syllabloom never receives a password, QR code, or cookie. The login return target preserves the current course and lecture; after Bilibili signs you in, it returns to that page, removes the one-time handoff parameters from the URL, and reloads the iframe automatically. **Reload in-page player** only reloads the official iframe; there is deliberately no app button that opens the full Bilibili website. The iframe is sandboxed without popup or top-navigation permission, so an action inside it cannot navigate the surrounding Syllabloom page away. In local validation of the CS336 source, the same 720p choice stayed at 360p in Bilibili's own external player while the full Bilibili page played 720p. This is a provider limitation that Syllabloom cannot override. Bilibili's own player can still display its “watch in Bilibili” prompt when a quality is unavailable in an embed; that provider decision cannot be overridden, but it cannot take the surrounding course page away. The in-page **Full screen** button expands the player without leaving the course workspace when the browser permits the Fullscreen API.
