...
Open **Add a course** — the Bilibili tab is selected by default. Paste a direct `BV...` video URL: the app reads public metadata and splits multi-part (multi-P) videos into one lecture per part (an optional course name overrides the Bilibili title). This creates a learner-selected, third-party source; it does not turn that video into official course material.

Playback runs in Syllabloom's own HTML5 player, so the quality menu lists exactly what Bilibili returns for your account and switches preserve the playback position. Choose **Scan to sign in** to display Bilibili's official QR code inside the page and scan it with the mobile app; anonymous playback is usually capped around 480P while signed-in playback reaches the entitlements of your own account (VIP tiers still require that membership). Session cookies are written only to the local SQLite database — never displayed or uploaded — and settings responses mask them as "configured". Watch progress counts toward course completion the same way YouTube lectures do.
